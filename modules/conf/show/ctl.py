...
def show_setting(**k):
	...
	glob= btrwin.modules.conf.load.ctl.global_config()
	if  k.get('file'):
		file = k['file']
		print(f'|->{file}/')
		config=glob[file]
...
def show_world_config(**k):
	world=k.get('W')
	k['c']=lib.conf.new() #use new configparser config as to not to replace the active one in glob config!
	sprint=sys.stdout.write
	c= btrwin.modules.conf.load.ctl.world_config(**k)
...
def show_global_config():
	...
	G= btrwin.modules.conf.load.ctl.global_config()
	def s_fx():
		"""
		is string_fix is string_prefix + string_suffix for config:show
		:return: list(f,s,o) where list(x) is tuple(prefix,suffix) for f=file,s=section,o=option
...
def show_rc_config(**k):
	...
	sprint=sys.stdout.write
	c= btrwin.modules.conf.load.ctl.running_config()
