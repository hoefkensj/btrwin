...
import debug as d
import os,datetime
from assets import skel
from subprocess import Popen,PIPE

def tpl_name(*a,**k):
		
		bit=k['bit']
		subv=conf.glob['betterwin']['PATH']['subv']
		exist= fs.btrfsctl.get_subvs(subv)
		prefix = 'tpl'
...
def link_loader(tpl,loader='wine',edition='lutris',version='default' ):
	PATHSYS=conf.glob['betterwin']['PATH']['sys']
...
def new_subvol(**k):
	name 		= k['name']
	subv		= conf.glob['betterwin']['PATH']['subv']
	before		= fs.btrfsctl.get_subvs(subv)
	tpl			= fs.btrfsctl.create_subv(subv, name)
	after		= fs.btrfsctl.get_subvs(subv)
	diff 		= [subvol for subvol in after if subvol not in before][0]
	return name if diff == tpl else None
	
def create_template(settings):
	
	SYS=conf.glob['betterwin']
	...
	conf.set(file=NAME, section='PATH', key='path', value=TPL)
	conf.set(file=NAME, section='PATH', key='loader', value=LLDR)
	# **os.environ
	#wine boot
	env = {
		'file'					:	f'{NAME}'	,
...
def main():
	...
	SYS_PATH=conf.glob['betterwin']['PATH']['sys']
	set={}
	set['BIT']=64
